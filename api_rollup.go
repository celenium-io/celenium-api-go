...
func (r ApiGetRollupProvidersRequest) Execute() ([]ResponsesRollupProvider, *http.Response, error) {
	return r.ApiService.GetRollupProvidersExecute(r)
}
...
// Execute executes the request
//  @return []ResponsesRollupProvider
func (a *RollupAPIService) GetRollupProvidersExecute(r ApiGetRollupProvidersRequest) ([]ResponsesRollupProvider, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  []ResponsesRollupProvider
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "RollupAPIService.GetRollupProviders")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/rollup/{id}/providers"
	localVarPath = strings.Replace(localVarPath, "{"+"id"+"}", url.PathEscape(parameterValueToString(r.id, "id")), -1)
